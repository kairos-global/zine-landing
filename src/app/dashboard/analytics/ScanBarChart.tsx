...
export function ScanBarChart({ data, title, height = 120, maxBars = 14 }: ScanBarChartProps) {
  ...
  if (display.length === 0) {
    return (
      <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4 text-center text-sm text-slate-500" style={{ minHeight: height }}>
      ...
      <div className="flex items-end gap-1" style={{ height }}>
        {display.map((p) => (
          <div key={p.date} className="flex-1 min-w-0 flex flex-col items-center justify-end gap-1">
            <div
              className="w-full bg-blue-500 rounded-t flex-shrink-0"
              style={{
                height: `${Math.max(4, Math.round((p.count / max) * (height - 20)))}px`,
              }}
