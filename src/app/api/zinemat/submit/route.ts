...
export async function POST(req: Request) {
  try {
    ...
    // Optionally capture Clerk email
    ...
    const { data: created, error: insErr } = await sb
      ...
      .insert({
        title: issue.title,
        slug: issue.slug,
        status: issue.status, // "draft" or "published"
        published_at: issue.published_at ? issue.published_at : null,
        created_by: userId,
        created_by_email: email, // now only included if Clerk has it
      })
      .select("id")
      .single();

    if (insErr || !created) {
      console.error("Issue insert error:", insErr);
      return NextResponse.json(
        { ok: false, error: { message: insErr?.message || "Failed to create issue." } },
        { status: 500 }
      );
    }
    ...
    console.error("Submit route fatal error:", err);
    return NextResponse.json(
